...
public class Controller_AddMO implements Initializable {
    ...
    private void M_Txfiled_SpSerialN_AddMO(ActionEvent event) {
        ...
        for (int i = 0; i < SPSelected2.size(); i++) {
            list2.add(new Controller_AddMO.SelectedSP(SPSelected2.get(i).getSP2_Number(), SPSelected2.get(i).getSP2_Name(), SPSelected2.get(i).getSP2_Description(), SPSelected2.get(i).getSP_Price2(), Txfiled_SpSerialN_AddMO.getText()));
    ...
    private void M_Btn_ReomveSP_AddMo(ActionEvent event) {
        ...
        /* for (int i = 0; i < SPSelected2.size(); i++) {
            //for(int i =1;i<=1; i++){
            //list.clear();
            list.add(new Controller_AddMO.AddSP(SPSelected2.get(i).getSP2_Number(), SPSelected2.get(i).getSP2_Name(), SPSelected2.get(i).getSP2_Description(), SPSelected2.get(i).getSP_Price2()));
    ...
    public void Search(String Search) {
        ...
        try {
            // نسحب الداتا من الداتا بييس  الى ان نخلص كل الداتا
            while (rs.next()) {
                // المتغيرات باللون الاخضر اسامي الاعمده فالداتا بيس عند انشائها
                ...
                // نظيف الداتا الي نسحبها عن طريق الليست , تضاف ف الكونستركتر حق البوك
    ...
    private void M_Btn_AddSP_AddMo(ActionEvent event) {
        ...
        for (int i = 0; i < SPSelected.size(); i++) {
            //for(int i =1;i<=1; i++){
            list2.add(new Controller_AddMO.SelectedSP(SPSelected.get(i).getSP_Number(), SPSelected.get(i).getSP_Name(), SPSelected.get(i).getSP_Description(), SPSelected.get(i).getSP_Price(), "null"));
            Table_SelectedSP_AddMO.setItems(list2);
    ...
    private void M_Btn_Search_AddMo(ActionEvent event) throws SQLException {
        ...
        System.out.println(Date_EndMO_AddMO.getValue());

        System.out.println(sql1);
        //String sql ="INSERT INTO spare_parts VALUES('"+Txfiled_ProplemDisc_AddMO.getText()+"')";
        //String sql1 = "INSERT INTO maintenance _operation  VALUES (NULL, 'ok', '50',
        //'60', '2019-01-15', '2019-01-15', '2019-01-15', 'hhhh', 'hhhh', 'hhhh', '1', '1', '2019-01-27', '100')";
        java.sql.Statement statement1 = connection.createStatement();
        statement1.executeUpdate(sql1);
         */
    }
    ...
    public void loadTech() {
        ...
        // هنا تسحب الداتا من الليست وتضاف ف الجدول

    }

    private void intilCol() {

        // لو حذفناها تحذف الخانات , يجي جدول فاضي
        // يعني الجدول لازم تعرف خاناته اول شي تعطيها اسامي ثم بعدين تضيف المتغيرات الى هذي الاسامي
        // في الكود ادناه , المتغيرات باللون الأخضر تجدها تحت معرفه في البوك , يعني نسوي اوبجكت منها
